<template>
	<el-dialog :title=title v-model="visible" :width="1000" destroy-on-close>
		<el-form :model="form" :rules="rules" ref="dialogForm" label-width="100px"
				 label-position="left">
			<el-form-item label="分类" prop="product_cate_id">
				<el-cascader v-model="form.product_cate_id" :options="options" :props="props"
			...
			<el-form-item label="文件" prop="video_url">
				<sc-upload-file v-model="form.video_url" :limit="1" :data="{otherData:'demo'}"
								tip="最多上传1个视频文件,单个文件不要超过10M,请上传文件">
					<el-button type="primary" icon="el-icon-upload">上传MP4/FLV文件</el-button>
				</sc-upload-file>
			</el-form-item>
	...
	methods: {
		...
		submit() {
			this.$refs.dialogForm.validate(async (valid) => {
				if (valid) {
					var res = await this.$API.website.product.save.post(this.form);
					if (res.code == 200) {
